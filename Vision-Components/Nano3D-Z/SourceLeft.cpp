...
SourceLeft::SourceLeft( uint32_t aSourceIndex )
...
void SourceLeft::GetWidthInfo( uint32_t &aMin, uint32_t &aMax, uint32_t &aInc ) const
{
    aMin = WIDTH_MIN;
    aMax = WIDTH_MAX;
    aInc = WIDTH_INC;
...
void SourceLeft::GetHeightInfo( uint32_t &aMin, uint32_t &aMax, uint32_t &aInc ) const
{
    aMin = HEIGHT_MIN;
    aMax = HEIGHT_MAX;
    aInc = HEIGHT_INC;
...
PvResult SourceLeft::GetSupportedPixelType( int aIndex, PvPixelType &aPixelType ) const
{
    switch ( aIndex )
    {
    case 0:
...
PvResult SourceLeft::SetWidth( uint32_t aWidth )
{
    mWidth = aWidth;
    return PvResult::Code::OK;
}
...
PvResult SourceLeft::SetHeight( uint32_t aHeight )
{
    mHeight = aHeight;
    return PvResult::Code::OK;
}
...
PvResult SourceLeft::SetPixelType( PvPixelType aPixelType )
{
...
PvBuffer *SourceLeft::AllocBuffer()
{
...
void SourceLeft::FreeBuffer( PvBuffer *aBuffer )
{
    PVDELETE( aBuffer );
    mBufferCount--;
}
...
PvResult SourceLeft::QueueBuffer( PvBuffer *aBuffer )
{
...
PvResult SourceLeft::RetrieveBuffer( PvBuffer **aBuffer )
{
    if ( mAcquisitionBuffer == NULL )
    {
...
void SourceLeft::AbortQueuedBuffers()
{
}
...
void SourceLeft::ResizeBufferIfNeeded( PvBuffer *aBuffer )
{
...
void SourceLeft::FillTestPatternMono8( PvBuffer *aBuffer )
{
    PvImage *lImage = aBuffer->GetImage();
